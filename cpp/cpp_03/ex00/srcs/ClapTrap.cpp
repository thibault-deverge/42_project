...
{
    std::cout << "Default constructor has been called : " << getName() << std::endl;
}

ClapTrap::ClapTrap(const std::string& name)
    : m_name(name) ,m_hit_points(10), m_energy_points(10), m_damage(0) 
{
    if (name.empty())
        m_name = ClapTrap::M_DEFAULT_NAME; 
    std::cout << "Parameterized constructor has been called : " << getName() << std::endl;
}

ClapTrap::ClapTrap(const ClapTrap& rhs) 
{ 
    *this = rhs; 
    std::cout << "Copy constructor has been called : " << getName() << std::endl;
}

ClapTrap::~ClapTrap() 
{
    std::cout << "Destructor has been called : " << getName() << std::endl;
}
...
void    ClapTrap::takeDamage(unsigned int amount)
{
    ...
    {
        m_hit_points -= amount;
        std::cout   << getName() << " has taken " << amount 
                    << " damages and still has " << getHitPoints() << " left.\n";
    }
}
